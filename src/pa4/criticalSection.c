...
void receiveAllRepliesHandler(BranchData *branchData, Request currentRequest, Workers workers) {
    ...
    for (int i = 0; i < workers.length; ++i) {
        if (branchData->id != workers.procId[i]) {
            while (true) {
                if (receive(branchData, workers.procId[i], &csReplies[i]) == 0) {
                    ...
                    } else if (csReplies[i].s_header.s_type == CS_REQUEST) {
                        receiveCsRequestAndSendReply(branchData, csReplies[i]);
                        continue;
                    } else if (csReplies[i].s_header.s_type == CS_RELEASE) {
                        receiveCsRelease(branchData, csReplies[i]);
                        continue;
                    } else if (csReplies[i].s_header.s_type == DONE) {
                        deleteWorker(branchData->senderId);
                        printf("in proc %d delete worker %d\n", branchData->id, branchData->senderId);
                        fflush(stdout);
                        break;
